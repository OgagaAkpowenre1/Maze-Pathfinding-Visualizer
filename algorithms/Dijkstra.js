...
export default class Dijkstra extends PathfinderBase {
   ...
   // In algorithms/Dijkstra.js - update executeStep method
executeStep() {
    ...
    for (const neighbor of neighbors) {
      ...
      if (!this.visited.has(neighborKey)) {
        // USE ACTUAL WEIGHT instead of fixed cost
        const cellWeight = this.gridManager.getCellWeight(neighbor.row, neighbor.col);
        const newDistance = this.distances.get(currentKey) + cellWeight;
        const currentDistance = this.distances.get(neighborKey);
        
        // If we found a shorter path to this neighbor
        if (newDistance < currentDistance) {
          console.log(`Updating distance for neighbor:`, neighbor, 
                     `from ${currentDistance} to ${newDistance} (weight: ${cellWeight})`);
